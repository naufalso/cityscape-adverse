...
do
    ...
    do
        ...
        # Evaluate the model
        CUDA_VISIBLE_DEVICES=3 python tools/test.py $model $checkpoint --work-dir work_dirs/acdc/${dataset}/$model_name \
        --cfg-options test_dataloader.dataset.type='ACDCDataset' \
        --cfg-options test_dataloader.dataset.data_root=data/acdc_processed/${dataset}/ \
